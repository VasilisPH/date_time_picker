...
class _DateTimePickerState extends FormFieldState<String> {
    ...
    if (lsValue != null && lsValue != '' && lsValue != 'null') {
      if (widget.type != DateTimePickerType.time) {
        ...
        if (widget.dateMask != null && widget.dateMask != '') {
          _dateLabelController.text = DateFormat(widget.dateMask, languageCode).format(_dDate);
        } else {
          ...
          _dateLabelController.text = DateFormat(lsMask, languageCode).format(_dDate);
        }
      } else {
  ...
  Future<void> _showDatePickerDialog() async {
    ...
    if (ldDatePicked != null) {
      ...
      if (widget.dateMask != null && widget.dateMask != '') {
        lsFormatedDate = DateFormat(widget.dateMask, languageCode).format(DateTime.tryParse(_sDate)!);
      } else {
        lsFormatedDate = DateFormat('MMM dd, yyyy', languageCode).format(DateTime.tryParse(_sDate)!);
      }
  ...
  void set12HourTimeValues(final TimeOfDay ptTimePicked) {
    ...
    final ldTime = DateTime(ldNow.year, ldNow.month, ldNow.day, ptTimePicked.hour, ptTimePicked.minute);
  ...
  Future<void> _showTimePickerDialog() async {
    final ltTimePicked = await showTimePicker(
      context: context,
      initialTime: _tTime,
      initialEntryMode: widget.timePickerEntryModeInput ? TimePickerEntryMode.input : TimePickerEntryMode.dial,
      useRootNavigator: widget.useRootNavigator,
      routeSettings: widget.routeSettings,
  ...
  Future<void> _showDateTimePickerDialog() async {
    ...
    if (ldDatePicked != null) {
      ...
      if (widget.dateMask != null && widget.dateMask != '') {
        lsFormatedDate = DateFormat(widget.dateMask, languageCode).format(DateTime.tryParse(_sValue)!);
      } else {
        final lsMask = _sTime != '' ? 'MMM dd, yyyy - HH:mm' : 'MMM dd, yyyy';
        lsFormatedDate = DateFormat(lsMask, languageCode).format(DateTime.tryParse(_sValue)!);
      }
